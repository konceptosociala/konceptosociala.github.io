...
**Warning:** this post is more like a cheat sheet than a full tutorial, so you should acquaint yourself with some basics
...
**Note**: you should also copy your `index.html` contents to `404.html` file as your server will redirect every page to a 404 default page, which would be useful to turn out to be a copy of the main app page :)
...
on [this pastebin link](https://pastebin.com/eGXvBERW), as I don't want to write the full code here ;P
...
3. `update` function: here we will process all the events emitted: 
   ...
   ```
   But let's also look at more interesting event:
   ```elm
